...
LOCAL unsigned short	pro_line_full[PRO_VID_SCRWIDTH];	/* single (unscaled) horizontal line - for Wii mode only */
LOCAL unsigned short	pro_line_out[PRO_VID_SCRHEIGHT][PRO_SCALE_OUTNUM];	/* horizontally-scaled framebuffer - for Wii mode only */

LOCAL int		pro_screen_window_scale_height;	/* Wii mode only: scaled vertical window size */
...
#endif
	  {
	    ...
	    if (XShmQueryExtension(ProDisplay))
	    {
	      ...
	      if (!(pro_image = XShmCreateImage(ProDisplay,
					      ...
					      NULL,
					      &shminfo,
					      PRO_VID_SCRWIDTH, PRO_VID_SCRHEIGHT*pro_screen_window_scale)))
	        shm_present = 0;
	      else if ((shminfo.shmid = 
		        shmget(IPC_PRIVATE, pro_image->bytes_per_line*PRO_VID_SCRHEIGHT*pro_screen_window_scale, IPC_CREAT|0777))==-1)
	        shm_present = 0;
...
/* XXX Hack for Wii */
	...
	pro_screen_window_scale_height = pro_screen_window_scale;
	pro_screen_window_scale = 2;
}
...
#ifdef SHM
                  {
/* TODO: optimize clipper, add pro_kernels to dependencies */

                    int ic, jc;
                    int r, g, b;
                    int startpix, coef, inpix, outpix;

                    /* Apply FIR filter */

                    for(ic=0; ic<PRO_SCALE_OUTNUM; ic++)
                    {
                      r = 0;
                      g = 0;
                      b = 0;

                      startpix = pro_kernels[ic][0];

                      for(jc=0; jc<PRO_SCALE_MAXTAPS; jc++)
                      {
/*
                        inpix = (int)((unsigned short *)image_data)[startpix+jc];
*/
                        inpix = (int)pro_line_full[startpix+jc];
                        coef = pro_kernels[ic][jc+1];
                        r += (int)(inpix & 0xf800) * coef;
                        g += (int)(inpix & 0x07e0) * coef;
                        b += (int)(inpix & 0x001f) * coef;
                      }
                      ...
                      pro_line_out[y][ic] = (short)outpix;
                    }
                  }
#endif
	        }
...
/* Update screen and perform vertical scaling (if pro_screen_full is not set) */

          {
            int ic, mvindex, mvindex_above, mvindex_below;
            int displayed_height, subpel, pix_above, pix_below, y_offset;
	    ...
	    if (pro_screen_full)
	    {
              ...
              {
                mvindex = cmem((pro_vid_y_offset+y*(PRO_VID_CLS_PIX/16)) & PRO_VID_VADDR_MASK);
	        if (pro_vid_mvalid[mvindex] == 0)
                {
	          image_data = pro_image_data + y*pro_screen_clsize;

                  for(ic=0; ic<PRO_SCALE_OUTNUM; ic++)
	            ((unsigned short *)image_data)[ic] = pro_line_out[y][ic];

		  XShmPutImage(ProDisplay, ProWindow, ProGC, pro_image, x, y,
		               x, y*pro_screen_window_scale, PRO_SCALE_OUTNUM, 1, False);

	          pro_vid_mvalid[mvindex] = 1;
                }
              }
	    }
	    else
	    {
              displayed_height = pro_screen_window_scale_height;

              y_offset = (PRO_VID_SCRHEIGHT * pro_screen_window_scale - displayed_height) / 2;

              for(y=0; y<displayed_height; y++)
              {
                subpel = (y*PRO_VID_SCRHEIGHT) % displayed_height;

                pix_above = (y*PRO_VID_SCRHEIGHT) / displayed_height;

                mvindex_above = cmem((pro_vid_y_offset+pix_above*(PRO_VID_CLS_PIX/16)) & PRO_VID_VADDR_MASK);

	        image_data = pro_image_data + y*pro_screen_clsize;

                /* If scaled and unscaled lines align, then simply copy */

	        if (subpel == 0)
                {
	          if (pro_vid_mvalid[mvindex_above] == 0)
                  {
                    for(ic=0; ic<PRO_SCALE_OUTNUM; ic++)
                      ((unsigned short *)image_data)[ic] = pro_line_out[pix_above][ic];

		    XShmPutImage(ProDisplay, ProWindow, ProGC, pro_image, x, y,
		                 x, y+y_offset, PRO_SCALE_OUTNUM, 1, False);
                  }
                }
                else
                {
                  pix_below = pix_above + 1;
	          if (pix_below == PRO_VID_SCRHEIGHT)
	            pix_below = PRO_VID_SCRHEIGHT - 1;

                  mvindex_below = cmem((pro_vid_y_offset+pix_below*(PRO_VID_CLS_PIX/16)) & PRO_VID_VADDR_MASK);

	          if ((pro_vid_mvalid[mvindex_above] == 0) || (pro_vid_mvalid[mvindex_below] == 0))
                  {
                    for(ic=0; ic<PRO_SCALE_OUTNUM; ic++)
	            {
                      r0 = (int)(pro_line_out[pix_above][ic] & 0xf800);
                      g0 = (int)(pro_line_out[pix_above][ic] & 0x07e0);
                      b0 = (int)(pro_line_out[pix_above][ic] & 0x001f);
                      r1 = (int)(pro_line_out[pix_below][ic] & 0xf800);
                      g1 = (int)(pro_line_out[pix_below][ic] & 0x07e0);
                      b1 = (int)(pro_line_out[pix_below][ic] & 0x001f);

	              r = (r1 * subpel + r0 * (displayed_height - subpel)) / displayed_height;
	              g = (g1 * subpel + g0 * (displayed_height - subpel)) / displayed_height;
	              b = (b1 * subpel + b0 * (displayed_height - subpel)) / displayed_height;
                      ...
                      ((unsigned short *)image_data)[ic] = (r | g | b);
	            }
		    ...
		    XShmPutImage(ProDisplay, ProWindow, ProGC, pro_image, x, y,
              ...
              /* Set all cache lines to valid */

              memset(&pro_vid_mvalid, 1, sizeof(pro_vid_mvalid));
	    }
          }
