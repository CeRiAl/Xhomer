...
/* XXX beginning of replaced code */
/* XXX Optimizing note:  The second versions of these shift
 * functions are probably overkill, but I just wanted to make
 * sure all the boundary conditions were met this time.  If (n)==0
 * never happens, it is ok to remove that case (stick a printf in
 * in there to find out, and do lots of graphics stuff, I guess.)
 * if (n)==64 never happens (this is likely) then same goes for it.
 * If .l and .h are always uint32's, I believe the &0xFFFFFFFF's are
 * not needed. GCC may be able to realize this when optimizing, but
 * you never know.
 *
 * For now I'm defining F_SH_FAST in the Makefile so the faster
 * version will run.
 */
#if 0
#ifdef F_SH_FAST
#define F_LSH_V(sr,n,ds) { \
  if ((n)>=32) { ds.l = 0; ds.h = sr.l << ((n) - 32); } \
  else /* 0<n<32 */ { ds.l = sr.l << (n); \
  		      ds.h = (sr.h << (n)) | (sr.l >> (32 - (n))); } \
}

#define F_RSH_V(sr,n,ds) { \
  if ((n)>=32) { ds.h = 0; ds.l = sr.h >> ((n) - 32); } \
  else /* 0<n<32 */ { ds.h = sr.h >> (n); \
                      ds.l = (sr.l >> (n)) | (sr.h << (32 - (n))); } \
}
#else
/*
 * These are the complete versions (use for debugging, if it
 * is suspected that a boundary condition is causing a problem.)
 */
#define F_LSH_V(sr,n,ds) { \
  if ((n)==0) { ds = sr; } \
  else if ((n)>=64) { ds.l = ds.h = 0; } \
  else if ((n)>=32) { ds.l = 0; ds.h = (sr.l << ((n) - 32))&0xFFFFFFFF; } \
  else /* 0<n<32 */ { ds.l = (sr.l << (n))&0xFFFFFFFF; \
  		      ds.h = ((sr.h << (n)) | (sr.l >> (32 - (n)))) \
			&0xFFFFFFFF; } \
}
...
#define F_RSH_V(sr,n,ds) { \
  if ((n)==0) { ds = sr; } \
  else if ((n)>=64) { ds.h = ds.l = 0; } \
  else if ((n)>=32) { ds.h = 0; ds.l = (sr.h >> ((n) - 32))&0xFFFFFFFF; } \
  else /* 0<n<32 */ { ds.h = (sr.h >> (n))&0xFFFFFFFF; \
                      ds.l = ((sr.l >> (n)) | (sr.h << (32 - (n)))) \
			&0xFFFFFFFF; } \
}
#endif
...
case 016:						/* LDCif */
	...
	fac.h = 0;
	if (fac.l) {
		if ((sign = GET_SIGN_L (fac.l))) fac.l = (fac.l ^ 0xFFFFFFFF) + 1; /* XXX added parentheses */
